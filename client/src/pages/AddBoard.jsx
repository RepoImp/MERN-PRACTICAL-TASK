...
const AddBoard = () => {
  const { serverOrigin, token, boards, getAllBoards } = useContext(AppContext);
  ...
  const addBoard = async (e) => {
    e.preventDefault();
    try {
      const res = await axios.post(`${serverOrigin}/board/create`, formData, {
  ...
  const deleteBoard = async (board_id) => {
    try {
      const res = await axios.delete(
        `${serverOrigin}/board/delete/${board_id}`,
        {
          headers: { "Content-Type": "application/json", token },
        }
      );
